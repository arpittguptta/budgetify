...
function SectionTitle({ title, span }) {
  ...
  return (
    <tr className="section-row">
      <td colSpan={span + 1} className="section-title">{title}</td>
    </tr>
  );
}

function SectionSpacer({ span }) {
  return (
...
/* ---------------- Investments tab helpers ---------------- */

// Field definitions for the Investments tab. Each header knows what it sorts by
// and what filter shape (if any) applies. Kept as data so the header rendering
// stays uniform and the sort/filter logic reads from one place.
const INV_FIELDS = [
  { key: "name",         label: "Name",              sortable: true, filterType: "text",     align: "left"   },
  { key: "type",         label: "Type",              sortable: true, filterType: "checklist", align: "left"   },
  { key: "invested",     label: "Invested (₹)",      sortable: true, filterType: "range",     align: "right"  },
  { key: "date",         label: "Date",              sortable: true, filterType: "daterange", align: "center" },
  { key: "currentValue", label: "Current Value (₹)", sortable: true, filterType: "range",     align: "right"  },
  { key: "targetPct",    label: "Target Profit (%)", sortable: true, filterType: null,        align: "right"  },
  { key: "stopPct",      label: "Stop Loss (%)",     sortable: true, filterType: null,        align: "right"  },
  { key: "gain",         label: "Gain / Loss",       sortable: true, filterType: null,        align: "right"  },
  { key: "status",       label: "Status",            sortable: true, filterType: "checklist", align: "left"   }
];

// Compute derived fields for a single row (gain amount + status) so sort/filter
// can reference them without recomputing per comparison.
function investmentDerived(r) {
  const invested = num(r.invested);
  const currentValue = num(r.currentValue);
  const gain = currentValue - invested;
  const status = investStatus(invested, currentValue, r.targetPct, r.stopPct);
  return { invested, currentValue, gain, status };
}

// Compare two rows for sort. field is one of INV_FIELDS[].key. direction 'asc' | 'desc'.
function compareInvestments(a, b, field, direction) {
  const da = investmentDerived(a);
  const db = investmentDerived(b);
  const dir = direction === "desc" ? -1 : 1;
  const strCmp = (x, y) => String(x || "").toLowerCase().localeCompare(String(y || "").toLowerCase()) * dir;
  const numCmp = (x, y) => (num(x) - num(y)) * dir;
  switch (field) {
    case "name":         return strCmp(a.name, b.name);
    case "type":         return strCmp(a.type, b.type);
    case "invested":     return numCmp(da.invested, db.invested);
    case "date":         return strCmp(a.date, b.date);
    case "currentValue": return numCmp(da.currentValue, db.currentValue);
    case "targetPct":    return numCmp(a.targetPct, b.targetPct);
    case "stopPct":      return numCmp(a.stopPct, b.stopPct);
    case "gain":         return numCmp(da.gain, db.gain);
    case "status": {
      // Severity order: Stop Loss Hit → Target Hit → Holding → "-"
      const order = { "Stop Loss Hit": 0, "Target Hit": 1, "Holding": 2, "-": 3 };
      return ((order[da.status] ?? 4) - (order[db.status] ?? 4)) * dir;
    }
    default: return 0;
  }
}

// Check if a row passes a single filter. Filter shapes:
//   text:      string (case-insensitive substring)
//   checklist: array of allowed values (empty array = no filter)
//   range:     { min, max } (either can be null)
//   daterange: { from, to } (YYYY-MM-DD strings)
function passesFilter(row, field, filter) {
  if (!filter) return true;
  const d = investmentDerived(row);
  switch (field) {
    case "name": {
    ...
    case "type": {
      ...
      return filter.includes(String(row.type || "").trim() || "—");
    }
    case "status": {
      if (!Array.isArray(filter) || filter.length === 0) return true;
      return filter.includes(d.status);
    }
    case "invested":
    case "currentValue": {
      const val = field === "invested" ? d.invested : d.currentValue;
      const min = filter.min !== null && filter.min !== undefined && filter.min !== "" ? num(filter.min) : null;
      const max = filter.max !== null && filter.max !== undefined && filter.max !== "" ? num(filter.max) : null;
...
// Whether a filter value is "empty" (i.e. equivalent to no filter set on that column).
function isFilterEmpty(field, filter) {
  if (filter === null || filter === undefined) return true;
  switch (field) {
    case "name": return !String(filter || "").trim();
    case "type":
    case "status": return !Array.isArray(filter) || filter.length === 0;
    case "invested":
    case "currentValue": {
      const min = filter.min;
      const max = filter.max;
      const minEmpty = min === null || min === undefined || min === "";
      const maxEmpty = max === null || max === undefined || max === "";
      return minEmpty && maxEmpty;
    }
    case "date": return !filter.from && !filter.to;
    default: return true;
  }
}

/* ---------------- Investments header popover ---------------- */

function InvHeaderPopover({ field, sortBy, setSortBy, filters, setFilters, allRows, onClose }) {
  const filterType = field.filterType;
  const currentFilter = filters[field.key];

  const setSortDir = (direction) => {
    setSortBy({ field: field.key, direction });
  };
  ...
  const applyFilter = (value) => {
    setFilters(prev => {
      const next = { ...prev };
      if (isFilterEmpty(field.key, value)) delete next[field.key];
  ...
  const isNumericField = ["invested", "currentValue", "targetPct", "stopPct", "gain"].includes(field.key);
  const isDateField = field.key === "date";
  const sortLabels = isNumericField
    ? { asc: "Low → High", desc: "High → Low" }
    : isDateField
      ? { asc: "Oldest → Newest", desc: "Newest → Oldest" }
      : field.key === "status"
        ? { asc: "Worst → Best", desc: "Best → Worst" }
        : { asc: "A → Z", desc: "Z → A" };

  // Build unique-value lists for checklist filters
  let uniqueValues = null;
  if (filterType === "checklist") {
    const counts = new Map();
    for (const r of allRows) {
      let v;
      if (field.key === "type") v = String(r.type || "").trim() || "—";
      else if (field.key === "status") v = investmentDerived(r).status;
      else v = "—";
      counts.set(v, (counts.get(v) || 0) + 1);
    }
    uniqueValues = Array.from(counts.entries()).sort((a, b) => a[0].localeCompare(b[0]));
  }

  const activeSort = sortBy && sortBy.field === field.key;
  const selectedList = Array.isArray(currentFilter) ? currentFilter : [];
  const rangeVal = currentFilter && !Array.isArray(currentFilter) && typeof currentFilter === "object" ? currentFilter : {};
  const textVal = typeof currentFilter === "string" ? currentFilter : "";

  return (
    <div className="inv-popover" onClick={e => e.stopPropagation()}>
      <div className="inv-pop-section">
        <div className="inv-pop-title">Sort</div>
        <div
          className={"inv-pop-item" + (activeSort && sortBy.direction === "asc" ? " active" : "")}
          onClick={() => setSortDir("asc")}
        >
          {activeSort && sortBy.direction === "asc" ? "✓ " : ""}{sortLabels.asc}
        </div>
        <div
          className={"inv-pop-item" + (activeSort && sortBy.direction === "desc" ? " active" : "")}
          onClick={() => setSortDir("desc")}
        >
          {activeSort && sortBy.direction === "desc" ? "✓ " : ""}{sortLabels.desc}
        </div>
        {activeSort && (
          <div className="inv-pop-item inv-pop-clear" onClick={clearSort}>Clear sort</div>
        )}
      </div>

      {filterType && (
        <>
          <div className="inv-pop-divider" />
          <div className="inv-pop-section">
            <div className="inv-pop-title">Filter</div>

            {filterType === "text" && (
              <input
                className="inv-pop-input"
                type="text"
            ...
            {filterType === "checklist" && uniqueValues && (
              <div className="inv-pop-checklist">
                {uniqueValues.length === 0 && <div className="inv-pop-empty">No values</div>}
                {uniqueValues.map(([val, count]) => {
                  const checked = selectedList.includes(val);
                  return (
                    <label key={val} className="inv-pop-chk">
                      <input
                        type="checkbox"
                        checked={checked}
                        onChange={e => {
                          const next = e.target.checked
                            ? [...selectedList, val]
                            : selectedList.filter(v => v !== val);
                          applyFilter(next);
                        }}
                      />
                      <span>{val || "—"}</span>
                      <span className="inv-pop-count">({count})</span>
                    </label>
                  );
                })}
              </div>
            ...
            {filterType === "range" && (
              <div className="inv-pop-range">
                <input
                  ...
                  placeholder="Min ₹"
                  value={rangeVal.min ?? ""}
                  onChange={e => applyFilter({ ...rangeVal, min: e.target.value })}
                />
                <input
                  className="inv-pop-input"
                  type="text"
                  ...
                  placeholder="Max ₹"
                  value={rangeVal.max ?? ""}
                  onChange={e => applyFilter({ ...rangeVal, max: e.target.value })}
                />
              </div>
            )}
            ...
            {filterType === "daterange" && (
              <div className="inv-pop-range">
                <input
                  ...
                  type="date"
                  value={rangeVal.from || ""}
                  onChange={e => applyFilter({ ...rangeVal, from: e.target.value })}
                />
                <input
                  className="inv-pop-input"
                  type="date"
                  value={rangeVal.to || ""}
                  onChange={e => applyFilter({ ...rangeVal, to: e.target.value })}
                />
              </div>
            )}

            {!isFilterEmpty(field.key, currentFilter) && (
              <div className="inv-pop-item inv-pop-clear" onClick={clearFilter}>Clear filter</div>
            )}
          </div>
        </>
      )}
      ...
      <div className="inv-pop-divider" />
      <div className="inv-pop-item inv-pop-close" onClick={onClose}>Close</div>
    </div>
  );
}
...
function InvestmentsTab({ month, setMonth, allMonths }) {
  ...
  const [sortBy, setSortBy] = useState(null);       // { field, direction } | null
  const [filters, setFilters] = useState({});       // { [fieldKey]: filterValue }
  const [openPopover, setOpenPopover] = useState(null); // fieldKey | null

  // Reset filters and sort whenever the user switches months. Each month is its own view.
  useEffect(() => {
    setSortBy(null);
    setFilters({});
    setOpenPopover(null);
  ...
  useEffect(() => {
    ...
    const onDocClick = () => setOpenPopover(null);
  ...
  const addRow = () => {
    // If filters are active, clear them so the newly-added blank row is visible.
    if (Object.keys(filters).length > 0) setFilters({});
    setMonth(prev => ({ ...prev, investments: [...(prev.investments || []), { name: "New holding", type: "Stocks", invested: null, date: "", currentValue: null, targetPct: null, stopPct: null }] }));
  };
  const removeRow = idx => setMonth(prev => ({ ...prev, investments: (prev.investments || []).filter((_, i) => i !== idx) }));

  // Build display list: each entry pairs the raw row with its ORIGINAL index in
  // month.investments so edit/delete still target the right underlying row.
  const withIndex = investments.map((row, originalIndex) => ({ row, originalIndex }));
  const filtered = withIndex.filter(({ row }) => {
    for (const [key, filter] of Object.entries(filters)) {
      if (!passesFilter(row, key, filter)) return false;
    }
    return true;
  });
  const sorted = sortBy
    ? [...filtered].sort((a, b) => compareInvestments(a.row, b.row, sortBy.field, sortBy.direction))
    : filtered;

  // Filtered totals — sum only visible rows so the Total row matches what's on screen.
  const filteredInvestedTotal = sorted.reduce((s, { row }) => s + num(row.invested), 0);
  const filteredCurrentTotal = sorted.reduce((s, { row }) => s + num(row.currentValue), 0);
  const filteredGain = filteredCurrentTotal - filteredInvestedTotal;
  const filteredGainPct = filteredInvestedTotal > 0 ? (filteredGain / filteredInvestedTotal) * 100 : 0;

  const activeFilterCount = Object.keys(filters).filter(k => !isFilterEmpty(k, filters[k])).length;
  const isFiltered = activeFilterCount > 0;

  const clearAll = () => { setFilters({}); setSortBy(null); };

  const investedTotalForDisplay = isFiltered ? filteredInvestedTotal : c.investedTotal;
  const currentTotalForDisplay = isFiltered ? filteredCurrentTotal : c.currentValueTotal;
  const gainForDisplay = isFiltered ? filteredGain : c.investmentGain;
  const gainPctForDisplay = isFiltered ? filteredGainPct : c.investmentGainPct;
    ...
    <div>
      {(isFiltered || sortBy) && (
        <div className="inv-status-banner">
          <span>
            ...
            {isFiltered && sortBy && <> · </>}
            {sortBy && <>Sorted by <b>{INV_FIELDS.find(f => f.key === sortBy.field)?.label}</b> {sortBy.direction === "asc" ? "↑" : "↓"}</>}
          </span>
          <button className="inv-clear-all" onClick={clearAll}>Clear all</button>
        </div>
      )}
      ...
      <SheetWrap cols={9}>
        ...
        <tr className="colhead-row">
          <td className="rownum" />
          {INV_FIELDS.map(f => {
            const hasFilter = !isFilterEmpty(f.key, filters[f.key]);
            const hasSort = sortBy && sortBy.field === f.key;
            const cls = "colhead inv-colhead"
              + (hasFilter ? " inv-filtered" : "")
              + (hasSort ? " inv-sorted" : "");
            return (
              <td
                key={f.key}
                ...
                onClick={(e) => {
                  e.stopPropagation();
                  setOpenPopover(openPopover === f.key ? null : f.key);
                }}
              >
                <span className="inv-colhead-label">{f.label}</span>
                {hasSort && <span className="inv-arrow">{sortBy.direction === "asc" ? "↑" : "↓"}</span>}
                {hasFilter && <span className="inv-filter-dot" aria-hidden="true">•</span>}
                {openPopover === f.key && (
                  <InvHeaderPopover
                    field={f}
                    sortBy={sortBy}
        ...
        {sorted.length === 0 && investments.length > 0 && (
          <tr>
            <td className="rownum" />
            <td colSpan={9} className="cell inv-empty-msg">
              No holdings match your filters. <button className="inv-clear-inline" onClick={clearAll}>Clear filters</button>
            </td>
          </tr>
        )}

        {sorted.map(({ row: r, originalIndex: i }) => {
          const invested = num(r.invested);
        ...
        <tr className="total-row">
          <RowNum />
          <Cell strong>{isFiltered ? "Total (filtered)" : "Total"}</Cell>
          <Cell />
          <Cell align="right" strong>{fmt(investedTotalForDisplay)}</Cell>
          <Cell />
          <Cell align="right" strong>{fmt(currentTotalForDisplay)}</Cell>
          <Cell /><Cell />
          <Cell align="right" strong>{investedTotalForDisplay > 0 ? `${fmt(gainForDisplay)} (${gainPctForDisplay.toFixed(1)}%)` : "-"}</Cell>
          <Cell />
        </tr>
        <tr>
          <RowNum />
  ...
  return (
    <div className="app-root">
      <style>{`
        ...
        .section-row .section-title { background: var(--surface-2); color: #8FB8E5; font-weight: 700; padding: 16px 14px; border: 1px solid var(--border-strong); letter-spacing: 0.08em; font-size: 14.5px; text-transform: uppercase; }
        .section-spacer td { border: none; background: transparent; height: 20px; padding: 0; }
        ...
        .inv-colhead { position: relative; cursor: pointer; user-select: none; }
        .inv-colhead:hover { background: var(--surface); color: var(--text); }
        .inv-colhead.inv-sorted { color: #8FB8E5; }
        .inv-colhead.inv-filtered { color: #8FB8E5; }
        .inv-colhead-label { display: inline-block; }
        .inv-arrow { display: inline-block; margin-left: 4px; font-size: 10px; color: #8FB8E5; }
        .inv-filter-dot { display: inline-block; margin-left: 4px; font-size: 14px; color: #8FB8E5; line-height: 1; vertical-align: middle; }
        .inv-status-banner { display: flex; justify-content: space-between; align-items: center; background: rgba(143,184,229,0.08); border: 1px solid rgba(143,184,229,0.3); color: #8FB8E5; font-size: 12px; padding: 7px 12px; border-radius: 6px; margin-bottom: 8px; }
        .inv-status-banner b { color: var(--text); font-weight: 500; }
        .inv-clear-all { background: transparent; border: none; color: var(--neg); font-size: 12px; cursor: pointer; font-family: inherit; padding: 2px 6px; }
        .inv-clear-all:hover { text-decoration: underline; }
        .inv-clear-inline { background: transparent; border: none; color: #8FB8E5; font-size: 12px; cursor: pointer; font-family: inherit; padding: 0 4px; text-decoration: underline; }
        .inv-empty-msg { text-align: center; padding: 14px !important; color: var(--text-dim); background: var(--surface); font-style: italic; }
        .inv-popover {
          position: absolute;
          top: 100%;
          left: 0;
        ...
        .inv-pop-section { padding: 4px 0; }
        .inv-pop-title { font-size: 10px; color: var(--text-dim); text-transform: uppercase; letter-spacing: 0.05em; padding: 4px 8px; font-weight: 500; }
        .inv-pop-item { padding: 7px 10px; font-size: 12.5px; color: var(--text-mid); cursor: pointer; border-radius: 4px; }
        .inv-pop-item:hover { background: var(--surface-2); color: var(--text); }
        .inv-pop-item.active { color: #8FB8E5; }
        .inv-pop-clear { color: var(--neg); font-size: 11.5px; }
        .inv-pop-close { text-align: center; color: var(--text-dim); font-size: 11.5px; }
        .inv-pop-divider { height: 1px; background: var(--border); margin: 4px 0; }
        .inv-pop-input { width: 100%; box-sizing: border-box; background: var(--bg); border: 1px solid var(--border-strong); color: var(--text-mid); font-size: 12px; padding: 6px 8px; border-radius: 4px; font-family: inherit; margin: 4px 0; color-scheme: dark; }
        .inv-pop-input:focus { outline: 1.5px solid #8FB8E5; border-color: #8FB8E5; }
        .inv-pop-range { display: grid; grid-template-columns: 1fr 1fr; gap: 6px; padding: 0 4px; }
        .inv-pop-range .inv-pop-input { margin: 4px 0; }
        .inv-pop-checklist { max-height: 180px; overflow-y: auto; padding: 4px 6px; }
        .inv-pop-chk { display: flex; align-items: center; gap: 8px; padding: 5px 4px; font-size: 12px; cursor: pointer; color: var(--text-mid); }
        .inv-pop-chk:hover { color: var(--text); }
        .inv-pop-chk input { accent-color: #8FB8E5; margin: 0; }
        .inv-pop-chk span { flex: 1; }
        .inv-pop-count { color: var(--text-dim); font-size: 11px; }
        .inv-pop-empty { padding: 8px 10px; color: var(--text-dim); font-size: 11.5px; font-style: italic; }
