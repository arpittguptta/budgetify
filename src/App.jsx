...
function PlannerApp() {
  ...
  // Only writes to months whose opening actually changed (to minimize Firestore writes).
  const cascadeOpeningBalances = useCallback(async (fromKey) => {
    ...
    for (let i = fromIdx + 1; i < all.length; i++) {
      ...
      // Replace in the local array so subsequent computeMonth uses the fresh value
      all[i] = patched;
      running = computeMonth(patched, all).closingBalance;
    }
    if (updated.length === 0) return;

    // Save all updated months in parallel
    await Promise.all(updated.map(m => saveMonthDoc(m)));
    // Update local state so the UI reflects new openings
    setAllMonths(prev => {
      const map = new Map(prev.map(m => [m.key, m]));
      updated.forEach(m => map.set(m.key, m));
      return Array.from(map.values());
    });
    // If we updated the currently-viewed month, refresh it too — BUT only if the user
    // isn't currently editing it (pendingMonthSave would indicate an in-flight edit).
