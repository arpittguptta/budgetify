...
// For a debt entry with an emiPlan, compute the outstanding balance as of a given month.
// If no emiPlan, returns whatever the user has stored in outstanding.
// Amortization: monthly interest on remaining principal, EMI covers interest + principal.
function effectiveOutstanding(debt, currentKey) {
  if (!debt || !debt.emiPlan) return num(debt?.outstanding);
  const plan = debt.emiPlan;
  const elapsed = monthsElapsed(plan.startKey, currentKey);
  if (elapsed <= 0) return num(plan.principal);
  if (elapsed >= num(plan.totalMonths)) return 0;
...
function buildNextMonth(prev, key) {
  ...
  // Fixed rows with an emiPlan drop off once the plan is complete.
  // Debt rows with an emiPlan drop off once fully paid.
  const emiActive = (r) => {
    if (!r.emiPlan) return true;
    const elapsed = monthsElapsed(r.emiPlan.startKey, key);
    return elapsed <= num(r.emiPlan.totalMonths);
  };
  return {
    key,
    income: { expected: prev.income.expected, actual: null },
    fixed: prev.fixed.filter(emiActive).map(r => ({
      name: r.name,
      budget: r.budget,
      actual: null,
    ...
    debts: prev.debts.filter(r => {
      if (!r.emiPlan) return true;
      return effectiveOutstanding(r, key) > 0;
    }).map(r => ({
      name: r.name,
...
function computeMonth(m) {
  ...
  const debtTotal = m.debts.reduce((s, r) => s + effectiveOutstanding(r, m.key), 0);
...
function DebtTab({ month, setMonth }) {
  ...
  const addRow = () => setMonth(prev => ({ ...prev, debts: [...prev.debts, { name: "New creditor", outstanding: null, type: "interest-free", interestRate: null }] }));
  const removeRow = idx => setMonth(prev => ({ ...prev, debts: prev.debts.filter((_, i) => i !== idx) }));

  const totalInterestFree = month.debts.filter(r => (r.type || "interest-free") === "interest-free").reduce((s, r) => s + effectiveOutstanding(r, month.key), 0);
  const totalInterestBearing = month.debts.filter(r => r.type === "interest-bearing").reduce((s, r) => s + effectiveOutstanding(r, month.key), 0);
  const monthlyInterestCost = month.debts.filter(r => r.type === "interest-bearing").reduce((s, r) => s + (effectiveOutstanding(r, month.key) * num(r.interestRate) / 100 / 12), 0);

  return (
    <SheetWrap cols={5}>
      <SectionTitle title="DEBT OUTSTANDING (WHAT I OWE)" span={5} />
      <HeaderRow labels={["Creditor", "Type", "Outstanding (₹)", "Interest Rate (% p.a.)", ""]} />
      {month.debts.map((r, i) => {
        const type = r.type || "interest-free";
        const hasEmiPlan = !!r.emiPlan;
        const eff = effectiveOutstanding(r, month.key);
        return (
          <tr key={i}>
            <RowNum />
            ...
            <Cell align="right">
              {hasEmiPlan ? (
                <span>{fmt(eff)} <span className="muted" style={{ fontSize: 10 }}>auto</span></span>
              ) : (
...
function LentTab({ month, setMonth }) {
  rowCounter = 1;
  const c = computeMonth(month);
...
function HomeTab({ allMonths, goals, wishlist, onGo }) {
  ...
  const c = computeMonth(last);

  // Net worth trend
  const netWorthSeries = sorted.map(m => {
    const cc = computeMonth(m);
    return { month: monthLabel(m.key), netWorth: cc.netWorth };
  });
  const prev = sorted.length >= 2 ? computeMonth(sorted[sorted.length - 2]) : null;
  ...
  const insights = generateInsights(sorted, goals);
      ...
      <div className="chart-box">
        <ResponsiveContainer width="100%" height={240}>
          <LineChart data={netWorthSeries}>
            <CartesianGrid strokeDasharray="3 3" stroke="#3B3F48" />
            <XAxis dataKey="month" tick={{ fontSize: 12, fill: "#8A8F9A" }} />
            <YAxis tick={{ fontSize: 12, fill: "#8A8F9A" }} tickFormatter={v => `₹${(v / 1000).toFixed(0)}k`} />
            <Tooltip formatter={v => fmt(v)} contentStyle={{ background: "#2A2D34", border: "1px solid #3B3F48", borderRadius: 6, color: "#E5E7EB", fontSize: 12 }} labelStyle={{ color: "#FAFAFA" }} />
            <Line type="monotone" dataKey="netWorth" name="Net worth" stroke="var(--text)" strokeWidth={2.5} dot={{ r: 3 }} />
          </LineChart>
        </ResponsiveContainer>
      </div>
    </div>
...
function currentFinancials(sortedMonths) {
  ...
  const avgSavings = last3.length
    ? last3.reduce((s, m) => s + computeMonth(m).savingsActual, 0) / last3.length
    : 0;
  const monthlyExpenses = last3.length
    ? last3.reduce((s, m) => s + computeMonth(m).expActual, 0) / last3.length
    : c.expActual;
  const debtInterest = (last.debts || [])
    .filter(d => d.type === "interest-bearing")
    .reduce((s, d) => s + (effectiveOutstanding(d, last.key) * num(d.interestRate) / 100 / 12), 0);
  const hasInterestBearingDebt = (last.debts || []).some(d => d.type === "interest-bearing" && effectiveOutstanding(d, last.key) > 0);
...
function HistoryTab({ allMonths }) {
  ...
  const ccColors = ["#D97757", "#9AB4C7", "#7FB88F", "#E8C46B", "#B79EDD", "#E88B7A"];
    ...
    <div className="history">
      ...
      <div className="chart-box">
        <ResponsiveContainer width="100%" height={260}>
          <LineChart data={rows}>
            <CartesianGrid strokeDasharray="3 3" stroke="#3B3F48" />
            <XAxis dataKey="shortLabel" tick={{ fontSize: 12, fill: "#8A8F9A" }} />
            <YAxis tick={{ fontSize: 12, fill: "#8A8F9A" }} tickFormatter={v => `₹${(v / 1000).toFixed(0)}k`} />
            <Tooltip formatter={v => fmt(v)} contentStyle={{ background: "#2A2D34", border: "1px solid #3B3F48", borderRadius: 6, color: "#E5E7EB", fontSize: 12 }} labelStyle={{ color: "#FAFAFA" }} />
            <Legend wrapperStyle={{ fontSize: 11, color: "#E5E7EB" }} />
            <Line type="monotone" dataKey="income" name="Income" stroke="#9AB4C7" strokeWidth={2} dot={{ r: 3 }} />
            <Line type="monotone" dataKey="expenses" name="Total expenses" stroke="#E88B7A" strokeWidth={2} dot={{ r: 3 }} />
            <Line type="monotone" dataKey="savings" name="Savings" stroke="var(--text)" strokeWidth={2} dot={{ r: 3 }} />
          </LineChart>
        </ResponsiveContainer>
      </div>
      ...
      <div className="chart-box">
        <ResponsiveContainer width="100%" height={260}>
          <LineChart data={rows}>
            ...
            <Tooltip formatter={v => fmt(v)} contentStyle={{ background: "#2A2D34", border: "1px solid #3B3F48", borderRadius: 6, color: "#E5E7EB", fontSize: 12 }} labelStyle={{ color: "#FAFAFA" }} />
            <Legend wrapperStyle={{ fontSize: 11, color: "#E5E7EB" }} />
            <Line type="monotone" dataKey="expenses" name="Total expense" stroke="#FAFAFA" strokeWidth={2} dot={{ r: 3 }} />
            <Line type="monotone" dataKey="nonCard" name="Non-card expense" stroke="#9AB4C7" strokeWidth={2} dot={{ r: 3 }} />
      ...
      <div className="chart-box">
        <ResponsiveContainer width="100%" height={260}>
          <ComposedChart data={ccData}>
            ...
            <Line type="monotone" dataKey="total" name="Total card spend" stroke="#FAFAFA" strokeWidth={2} dot={{ r: 3 }} />
          </ComposedChart>
        </ResponsiveContainer>
      </div>
      ...
      <div className="chart-box">
        <ResponsiveContainer width="100%" height={240}>
          <LineChart data={catData}>
            <CartesianGrid strokeDasharray="3 3" stroke="#3B3F48" />
            <XAxis dataKey="month" tick={{ fontSize: 12, fill: "#8A8F9A" }} />
            <YAxis tick={{ fontSize: 12, fill: "#8A8F9A" }} tickFormatter={v => `₹${(v / 1000).toFixed(1)}k`} />
            <Tooltip formatter={v => fmt(v)} contentStyle={{ background: "#2A2D34", border: "1px solid #3B3F48", borderRadius: 6, color: "#E5E7EB", fontSize: 12 }} labelStyle={{ color: "#FAFAFA" }} />
            <Legend wrapperStyle={{ fontSize: 11, color: "#E5E7EB" }} />
            <Line type="monotone" dataKey="budget" name="Budget" stroke="#E8C46B" strokeWidth={2} dot={{ r: 3 }} />
            <Line type="monotone" dataKey="actual" name="Actual" stroke="var(--text)" strokeWidth={2} dot={{ r: 3 }} />
          </LineChart>
        </ResponsiveContainer>
      </div>
      ...
      <div className="chart-box">
        <ResponsiveContainer width="100%" height={240}>
          <LineChart data={rows}>
            ...
            <Tooltip formatter={v => fmt(v)} contentStyle={{ background: "#2A2D34", border: "1px solid #3B3F48", borderRadius: 6, color: "#E5E7EB", fontSize: 12 }} labelStyle={{ color: "#FAFAFA" }} />
            <Legend wrapperStyle={{ fontSize: 11, color: "#E5E7EB" }} />
            <Line type="monotone" dataKey="balance" name="Closing balance" stroke="var(--text)" strokeWidth={2} dot={{ r: 3 }} />
      ...
      <div className="chart-box">
        <ResponsiveContainer width="100%" height={240}>
          <LineChart data={rows}>
            ...
            <Legend wrapperStyle={{ fontSize: 11, color: "#E5E7EB" }} />
            <Line type="monotone" dataKey="netWorth" name="Net worth" stroke="var(--text)" strokeWidth={2.5} dot={{ r: 3 }} />
            <Line type="monotone" dataKey="balance" name="Savings balance" stroke="#9AB4C7" strokeWidth={1.5} dot={{ r: 2 }} strokeDasharray="3 3" />
          </LineChart>
        </ResponsiveContainer>
      </div>
...
function Login() {
  ...
  return (
    <div className="login-wrap">
      <style>{`
        .login-wrap { min-height: 60vh; display: flex; align-items: center; justify-content: center; font-family: 'Söhne', 'Inter', -apple-system, sans-serif; background: #22252B; padding: 20px; }
        .login-form { background: #2A2D34; border: 1px solid #2E323A; border-radius: 10px; padding: 32px 28px; width: 300px; }
        .login-form h2 { margin: 0 0 20px; font-size: 16px; color: #FAFAFA; font-weight: 500; letter-spacing: 0.01em; display: flex; align-items: center; gap: 10px; }
        .login-form h2::before { content: ""; width: 18px; height: 18px; background: #D97757; border-radius: 5px; display: inline-block; }
        .login-form input { width: 100%; box-sizing: border-box; padding: 10px 12px; margin-bottom: 10px; border: 1px solid #3B3F48; border-radius: 6px; font-family: inherit; font-size: 13px; background: #22252B; color: #E5E7EB; }
        .login-form input:focus { outline: 1.5px solid #D97757; border-color: #D97757; }
        .login-form button { width: 100%; padding: 10px; background: #D97757; color: #fff; border: none; border-radius: 6px; font-size: 13px; cursor: pointer; font-family: inherit; font-weight: 500; }
        .login-form button:hover { background: #C46A4C; }
...
function PlannerApp() {
  ...
  // Flush any pending debounced save right now. Safe to call at any time.
    ...
    <div className="app-root">
      <style>{`
        .app-root {
          ...
          --warn: #E8C46B;
          --sheet-bg: #2A2D34;
          --sheet-header-bg: #33363E;
          --sheet-header-text: #E5E7EB;
          --input-focus-bg: rgba(217,119,87,0.05);
          font-family: 'Söhne', 'Inter', -apple-system, 'Segoe UI', system-ui, sans-serif;
          color: var(--text-mid);
          background: var(--bg);
          padding: 12px;
          border-radius: 10px;
        ...
        .app-loading { padding: 40px; text-align: center; color: var(--text-dim); background: var(--bg); border-radius: 10px; font-family: 'Söhne', 'Inter', sans-serif; }
        .topbar { display: flex; flex-wrap: wrap; align-items: center; gap: 10px; margin-bottom: 12px; padding: 4px 8px 0; }
        .topbar h1 { font-size: 15px; font-weight: 500; color: var(--text); margin: 0; flex: 1 1 100%; letter-spacing: 0.01em; display: flex; align-items: center; gap: 10px; }
        .topbar h1::before { content: ""; width: 18px; height: 18px; background: var(--accent); border-radius: 5px; display: inline-block; }
        .tabs-row { display: flex; flex-wrap: wrap; gap: 4px; flex: 1 1 auto; margin-top: 6px; padding: 0 8px; border-bottom: 1px solid var(--border); }
        .tabbtn { border: 1px solid transparent; background: transparent; padding: 8px 14px; font-size: 12px; cursor: pointer; border-radius: 6px 6px 0 0; font-family: inherit; color: var(--text-dim); margin-bottom: -1px; }
        .tabbtn:hover { color: var(--text-mid); }
        .tabbtn.active { color: var(--text); background: var(--surface); border-color: var(--border); border-bottom-color: var(--surface); }
        .monthselect { padding: 6px 10px; font-size: 12px; border: 1px solid var(--border-strong); border-radius: 6px; font-family: inherit; background: var(--surface); color: var(--text-mid); }
        .newmonthbtn { border: 1px solid var(--accent); background: var(--accent); color: #fff; padding: 6px 12px; font-size: 12px; border-radius: 6px; cursor: pointer; font-family: inherit; font-weight: 500; }
        .newmonthbtn:hover { background: var(--accent-hover); }
        .signoutbtn { border: 1px solid var(--border-strong); background: var(--surface); color: var(--text-dim); padding: 6px 12px; font-size: 12px; border-radius: 6px; cursor: pointer; font-family: inherit; }
        .signoutbtn:hover { color: var(--text-mid); border-color: var(--text-faint); }
        .signoutbtn:disabled { opacity: 0.4; cursor: not-allowed; }
        .savebtn { border: 1px solid var(--border-strong); background: var(--surface-2); color: var(--text); padding: 6px 14px; font-size: 12px; border-radius: 6px; cursor: pointer; font-family: inherit; font-weight: 500; }
        .savebtn:hover { background: var(--border-strong); }
        .muted { color: var(--text-faint); }
        .stat-card { background: var(--surface); border: 1px solid var(--border); border-radius: 10px; padding: 14px 16px; cursor: pointer; transition: border-color 0.15s; }
        .stat-card:hover { border-color: var(--border-strong); }
        .stat-hero { grid-column: span 2; background: var(--surface); border-color: var(--border); }
        .stat-value-big { font-size: 32px; font-weight: 500; color: var(--text); letter-spacing: -0.02em; }
        ...
        .insight { display: flex; align-items: center; gap: 10px; background: var(--surface); border-radius: 8px; padding: 10px 14px; font-size: 12.5px; color: var(--text-mid); border-left: 2px solid var(--accent); }
        .insight-good { border-left-color: var(--pos); }
        .insight-warn { border-left-color: var(--accent); }
        .insight-dot { width: 5px; height: 5px; border-radius: 50%; background: var(--text-faint); flex-shrink: 0; display: none; }
        ...
        .letter-row .corner, .letter-row .letter { background: var(--sheet-header-bg); border: 1px solid var(--border); text-align: center; font-weight: 500; color: var(--text-faint); padding: 4px 0; position: sticky; top: 0; z-index: 2; font-size: 11px; }
        ...
        .section-row .section-title { background: var(--surface-2); color: var(--text); font-weight: 500; padding: 8px 12px; border: 1px solid var(--border-strong); letter-spacing: 0.02em; font-size: 12px; }
        ...
        .cell { border: 1px solid var(--border); padding: 4px 8px; height: 28px; white-space: nowrap; }
        ...
        .badge-green { background: rgba(127,184,143,0.15); color: var(--pos); }
        .badge-amber { background: rgba(232,196,107,0.15); color: var(--warn); }
        .badge-red { background: rgba(232,139,122,0.15); color: var(--neg); }
        .badge-gray { background: rgba(138,143,154,0.15); color: var(--text-dim); }
        ...
        .history { background: var(--surface); border: 1px solid var(--border); padding: 18px; border-radius: 8px; }
        ...
        .hist-heading { font-size: 12.5px; font-weight: 500; color: var(--text); margin: 20px 0 10px; letter-spacing: 0.02em; }
        ...
        .funding-recommended-badge { background: var(--accent); color: #fff; font-size: 10px; padding: 2px 9px; border-radius: 10px; font-weight: 500; }
        ...
        .modal-backdrop { position: fixed; top: 0; left: 0; right: 0; bottom: 0; background: rgba(14,17,22,0.65); display: flex; align-items: center; justify-content: center; z-index: 100; padding: 16px; }
      ...
      {view === "expenses" && <ExpensesTab month={month} setMonth={setMonthWithUndo} />}
      {view === "savings" && <SavingsTab month={month} setMonth={setMonthWithUndo} />}
      {view === "debt" && <DebtTab month={month} setMonth={setMonthWithUndo} />}
      {view === "lent" && <LentTab month={month} setMonth={setMonthWithUndo} />}
      {view === "investments" && <InvestmentsTab month={month} setMonth={setMonthWithUndo} />}
      {view === "goals" && (
